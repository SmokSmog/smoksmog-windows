#include "pch.h"
#include "TilesBackgroundTask.h"

using namespace SmokSmog::Notification;
...
//	{
TilesBackgroundTask::TilesBackgroundTask()
{
}
...
void TilesBackgroundTask::OnRun(IBackgroundTaskInstance^ taskInstance)
{
	Agile<BackgroundTaskDeferral^> deferral = Agile<BackgroundTaskDeferral^>(taskInstance->GetDeferral());

	//SmokSmog

	create_task(GenerateHighResTileImageUpdateAsync("Medium.xml", "MediumTile.png", Size(150, 150)))
		.then([this, deferral]()
	{
		//UpdateTile("MediumTile.png");
...
task<void> TilesBackgroundTask::GenerateHighResTileImageUpdateAsync(String^ inputMarkupFilename, String^ outputImageFilename, Size size)
{
	//auto uri = ref new System::Uri();
	//create_task(Windows::Storage::StorageFile::GetFileFromApplicationUriAsync());

	auto client = ref new Windows::Web::Http::HttpClient();
	auto baseUri = ref new Windows::Foundation::Uri("localhost");
	auto resultStorageFolder = Package::Current->InstalledLocation;


	auto _task = create_task(client->GetStringAsync(baseUri));
	...
	//return create_task(Package::Current->InstalledLocation->GetFolderAsync("SmokSmog.Core\\Controls\\"))
	...
	//	Border^ control = (Border^)XamlReader::Load(markupContent);
	...
	//	control->Background = GetRandomBackgroundBrush();
	...
	//	return RenderAndSaveToFileAsync(control, outputImageFilename, (unsigned int)size.Width, (unsigned int)size.Height);
	//});
}

task<void> TilesBackgroundTask::RenderAndSaveToFileAsync(UIElement^ uiElement, String^ outputImageFilename, uint32 width, uint32 height)
{
...
Array<unsigned char>^ TilesBackgroundTask::GetArrayFromBuffer(IBuffer^ buffer)
{
...
void TilesBackgroundTask::StorePixelsFromBuffer(IBuffer^ buffer)
{
	this->pixelData = GetArrayFromBuffer(buffer);
}

task<void> TilesBackgroundTask::WriteBufferToFile(String^ outputImageFilename)
{
	auto resultStorageFolder = Package::Current->InstalledLocation;
...
void TilesBackgroundTask::UpdateTile(String^ tileUpdateImagePath)
{
...
String^ TilesBackgroundTask::GetCurrentTime()
{
...
String^ TilesBackgroundTask::GetCurrentTimezone()
{
	Calendar^ calendar = ref new Calendar();
...
Brush^ TilesBackgroundTask::GetRandomBackgroundBrush()
{
